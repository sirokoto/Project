...
class Player:
    def __init__(self, screen):
        ...
        self.rect.centerx = SCREEN_WIDTH//2-48
        self.rect.bottom = SCREEN_HEIGHT//2+250
        self.on_the_left_side = True
    ...
    def update_player(self):
        ...
        if keys[pygame.K_SPACE]:
            ...
            if self.on_left_side:
                self.rect.centerx = SCREEN_WIDTH // 2 - 48  # Перемещение на левую сторону
                self.image_to_draw = self.image  # Обычное изображение
            else:
                self.rect.centerx = SCREEN_WIDTH // 2 + 48  # Перемещение на правую сторону
                self.image_to_draw = self.flipped_image  # Отраженное изображение

    def draw(self):
        if not self.on_the_left_side:
    ...
    def toggle_side(self):
        self.on_the_left_side = not self.on_the_left_side

        if self.on_the_left_side:
